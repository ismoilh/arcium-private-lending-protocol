...
import { LendingService, LoanApplication, LoanOffer, ActiveLoan } from './lending.service';
...
export class LendingController {
  ...
  @ApiOperation({ summary: 'Submit a loan application with encrypted parameters' })
  @ApiBody({ type: SubmitLoanApplicationDto })
  @ApiResponse({ status: 201, description: 'Loan application submitted successfully' })
  async submitLoanApplication(@Body() params: SubmitLoanApplicationDto): Promise<LoanApplication> {
  ...
  async createLoanOffer(@Body() offerData: CreateLoanOfferDto): Promise<LoanOffer> {
  ...
  async acceptLoanOffer(@Body() acceptData: AcceptLoanOfferDto): Promise<ActiveLoan> {
    return this.lendingService.acceptLoanOffer(acceptData.offerId, acceptData.borrowerId);
  }
  ...
  async getBorrowerApplications(@Param('borrowerId') borrowerId: string): Promise<LoanApplication[]> {
  ...
  async getLenderOffers(@Param('lenderId') lenderId: string): Promise<LoanOffer[]> {
  ...
  async getUserActiveLoans(@Param('userId') userId: string): Promise<ActiveLoan[]> {
  ...
  async getLoanApplicationDetails(@Param('applicationId') applicationId: string) {
